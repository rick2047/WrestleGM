...
- Rivalry progression is 100% when a pair appears in a match and is not in cooldown; each appearance increments `rivalry_value` by 1 up to a cap of 4. The rivalry level is derived as `rivalry_level = min(4, rivalry_value)`.
