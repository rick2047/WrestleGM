## 1. Implementation
- [x] 1.1 Add Textual UI flow tests for core journeys (new game, booking show, results loop).
- [x] 1.2 Add UI snapshot test harness and generate baseline SVGs for canonical screens.
- [x] 1.3 Enforce snapshot registry checks (no missing or extra snapshots).
- [x] 1.4 Update CI workflow to run flow tests before snapshot tests and upload failing snapshots.
- [x] 1.5 Document snapshot update workflow for contributors.

## 2. Validation
- [x] 2.1 Run UI flow tests locally.
- [x] 2.2 Run UI snapshot tests locally and confirm deterministic output.
